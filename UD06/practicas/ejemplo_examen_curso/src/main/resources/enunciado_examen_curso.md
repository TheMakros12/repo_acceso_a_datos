...
1. **Crear un nuevo Curso**
   - `POST /api/cursos`
   - Body: **`CursoRequestDTO`** (datos del curso a crear, sin ID).
   - Retorna: **`CursoResponseDTO`** y estado `201 Created`.

2. **Listar todos los Cursos disponibles**
   - `GET /api/cursos`
   - Retorna: Lista de **`CursoResponseDTO`**. Estado `200 OK`.

3. **Matricular a un Estudiante en un Curso**
   - `POST /api/cursos/{cursoId}/estudiantes`
   - Path variable: `cursoId`.
   - Body: **`EstudianteRequestDTO`** (datos del estudiante a matricular, sin ID).
   - Lógica: Verificar que el curso exista. Si no existe, devolver `404 Not Found`.
   - Retorna: **`EstudianteResponseDTO`** y estado `201 Created`.

4. **Obtener el listado de Estudiantes de un Curso**
   - `GET /api/cursos/{cursoId}/estudiantes`
   - Path variable: `cursoId`.
   - Retorna: Lista de **`EstudianteResponseDTO`**. Si el curso no existe, `404 Not Found`. Estado `200 OK`.

5. **Buscar un Estudiante por su Email**
   - `GET /api/estudiantes/busqueda?email={email}`
   - Query parameter: `email`.
   - Retorna: **`EstudianteResponseDTO`** si se encuentra, o un `404 Not Found` si no existe ningún estudiante con ese correo.

6. **Actualizar la información de un Estudiante**
   - `PUT /api/estudiantes/{id}`
   - Path variable: `id` del estudiante.
   - Body: **`EstudianteRequestDTO`** (todos los datos del estudiante, sin ID).
   ...
   - Retorna: **`EstudianteResponseDTO`** actualizado y estado `200 OK`.
